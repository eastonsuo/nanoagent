...
本部分按概念组织，目标是先把「agent 由哪些部分组成」「nanoagent 的设计赌注是什么」讲清楚，再进入实现。
...
第 3.1 节提出了「核心稳定、策略可插拔」这个赌注，本章回答它在代码层面**靠什么机制成立**。
...
**当前方案为何如此** ：把 harness 能力做成 Hook 注入，代价是引入了一层间接性。替代方案是核心层直接 import 并调用各策略，代码更直白，但那样核心层就与策略层耦合，第 3.1 节的赌注直接失效。nanoagent 选择承担这层间接性，并用一个缓解措施抵消它的学习成本 ：v0.1 的默认配置不带任何 hook，初学者第一次读到的核心循环就是纯粹的 ReAct。
...
「稳定核心」是个赌注，必须在 v0.1 就验证它扛得住最硬的场景——上下文工程。这里用一个**假想的 v0.3 压缩策略**压测一遍：实现「保留预算内的最近消息、更早的折叠出视图」，看 core 是否真的一行不改。
...
**自检结论**：接入这个压缩策略，`core/loop.py`、8 个 Hook 签名、`Message/Context` 字段**一行都不改**——新代码全部落在 `strategies/`。这证明赌注在上下文工程这个最硬的场景下成立；也正是为此，§5.1 的 `view()` 与那几个占位字段必须在 v0.1 就位——它们是「core 不改」的物理前提。
...
`core/` 不得 import 任何外层目录（`strategies` / `tools` / `llm` / `memory` / `cli` / `api`）——这是「稳定核心」赌注的唯一物理保证。注意：策略 Protocol（`ContextStrategy` / `PermissionStrategy` / `StopStrategy`）属于**契约**，放在 `core/protocols.py`，故 `core` 依赖它们不算违规（依赖指向 core 自身）；策略的**实现**才在 `strategies/`。
...
## 10. v0.1 详细设计 ：从空项目到能跑

> 本章是 v0.1 的**高层概览**（范围 / 模块依赖 / 一周顺序）。**实现级详细设计**（逐模块签名、`@tool`→schema 生成算法、loop 30 行逐行走读、api/cli 装配、测试矩阵）已独立成文，见 [`v0.1-design.md`](v0.1-design.md)。

### 10.1 v0.1 的范围边界

v0.1 要做对的是「可演进性」，不是「功能完整」。核心循环、Hook 协议、能力契约这三者的形状必须一次定对 ；默认的 LLM 客户端、memory、策略可以最简陋。判断 v0.1 是否成功的标准是 ：v0.3 引入真正的 harness 时，`core/` 目录一行都不用改。

### 10.2 模块依赖关系

下图回答 ：「v0.1 各模块之间 import 谁、依赖方向是否单向？」
...
`顶层 API 装配层` 是唯一「知道全部模块」的地方，它依赖面最广，但它不在 `core/` 内。

### 10.3 实现顺序（按依赖推进，不绑定周期）

| 顺序 | 任务 | 完成判据 |
|---|---|---|
| 1 | `core/` 全部 ：数据结构、能力契约、Hook 契约 | 类型检查通过，契约定义完整 |
| 2 | `tools/` ：`@tool` 装饰器 + 自动 schema 生成 | 能注册内置工具，schema 与 OpenAI 格式一致 |
| 3 | `llm/` + `memory/` + 默认策略 | 能跑通最小循环（不开 hook） |
| 4 | `core/loop.py` ：`AgentLoop` 完整实现 | 第一个示例（hello agent）跑通 |
| 5 | `api.py` + `cli/` | `nanoagent` 命令可启动 REPL 并对话 |
| 6 | 补齐覆盖率 + 第二个示例（research agent） | 测试覆盖 > 70%（单测已随 1–5 各步同步写） |
| 7 | README + 整体走查 | 能在 30 分钟内讲清 v0.1 |

> **「顺序」是依赖序、不是日历**：上表序号表示先后依赖，不绑定「第几天」、不强制一周完成。**单测与每步代码同步写**（echo 客户端驱动、本地可跑、不依赖网络/API key），不堆到最后；第 6 步只是补齐覆盖与整体走查。详见 [`v0.1-design.md`](v0.1-design.md) §10。
