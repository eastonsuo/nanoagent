...
**读图要点** ：暖黄的**核心层**是地基，所有箭头都指向它 ；**能力实现层**与**策略层**各实现 core 的一类契约——前者（Tool / LLM / Memory）被循环直接调用（其中 Memory 略特殊 ：它属能力契约，但运行时**经 Tool 系统访问**、不落在 ReAct 主干上，详见 §6 读图要点），后者经 Hook 注入 ；**入口与装配**把它们组装成可运行的 agent ；**扩展与配套**多为工具形态、挂在 Tool 系统之上。颜色只区分大类、与版本无关——某大类下各模块在哪个版本落地，见下方各表的「阶段」列。
...
贯穿全文的「契约」一词，指 core 定义的抽象规范。**契约是抽象的协议 / 规范**，只规定「实现必须满足什么」，与用什么语言机制表达无关。在 core 里，契约有两种落地方式 ：**`typing.Protocol`** 表达「行为接口」（该有哪些方法），**`@dataclass`** 表达「数据形状」（该有哪些字段）。这两样全篇大量用到、不熟会卡，下面各给一段速成（完整签名见第 5 章）。此外还有一类两者都表达不了的**行为约定**（如「`view()` 不得改 `messages`」「`tool` 消息必须带 `tool_call_id` 配对」），靠测试与 CI 依赖防线（§8.1）来守。

#### 1.5.1 `typing.Protocol`——表达「行为接口」

它是 Python 的**结构化类型**（俗称「静态鸭子类型」）。一个类**只要有签名对得上的方法，就自动满足该协议，无需 `class X(协议)` 去继承** ：

```python
class StopStrategy(Protocol):              # 契约：要有这个方法
    def should_stop(self, ctx, turn) -> StopReason | None: ...

class MaxTurnsStop:                        # 注意：没写 (StopStrategy)
    def should_stop(self, ctx, turn): ...  # 但签名对上了 → 它就是合格的 StopStrategy

AgentLoop(stop=MaxTurnsStop())             # 类型检查器在「这一行」校验它满足 StopStrategy
```

三点要记住 ：① **无需继承**，方法对上即可（本框架的工具 / 客户端 / 策略实现都不继承对应 Protocol）；② 校验发生在**使用点**（传进去那一刻），不在类定义处；③ 你**也可以**显式继承（`class MaxTurnsStop(StopStrategy)`），好处是拼错方法名能更早报错、更自文档化，代价是多一层 import——本框架默认不继承，以凸显「core 与实现解耦」。多个实现还能**组合**进一个同样满足协议的类（如 `CompositeStop`），照样塞进单个参数（见 §5.4）。

#### 1.5.2 `@dataclass`——表达「数据形状」

`@dataclass`（标准库 `dataclasses`）让你**只声明字段（名字 + 类型 + 默认值），就自动得到 `__init__` / `__repr__` / `__eq__`**——一个带类型的「数据袋」，零样板。这组字段**本身就是契约** ：
...
ToolCall(id="1", name="read_file", arguments={})                  # __init__ 自动生成
ToolCall("1", "read_file", {}) == ToolCall("1", "read_file", {})  # __eq__ 按字段比，自动有
```

两点要记住 ：① 可变默认值必须用 **`field(default_factory=list)`**，不能直接写 `= []`（否则 dataclass 当场报错，替你挡掉「共享可变默认值」的经典坑）；② **`@dataclass` 不做运行时校验**——类型只是注解，塞个不符类型的值它不拦。这正是 §3.2「不引 pydantic、core 不做校验」的取舍 ：守住核心依赖 ≤5、保持极简，宁可简陋也不堆抽象。core 的全部数据结构（`Message` / `Context` / `ToolResult` …，见 §5.1）都是这样的纯 `@dataclass`、零第三方依赖。
...
```mermaid
flowchart TD
    subgraph outside["core 之外 · 可插拔（依赖单向指向 core）"]
        capimpl["能力实现<br/>Tool · LLM · Memory"]
        stratimpl["策略实现<br/>上下文 · 权限 · 停止"]
    end
    ...
    capimpl -.->|"实现"| capproto
    stratimpl -.->|"实现"| stratproto
    loop -->|"直接调用"| capproto
    loop -->|"运行时回调"| hookapi
    hookapi -.->|"在生命周期点触发被注入的策略"| stratproto

    classDef stable fill:#fff0aa
    class loop,capproto,stratproto,hookapi stable
```

暖黄节点是核心层。**能力契约与策略契约是平行的两族核心契约**——都由 core 之外的实现来满足（图中「能力实现 → 能力契约」「策略实现 → 策略契约」两条对称）。二者差别只在**访问路径**、不在层级 ：循环**直接调用**能力契约（实线）；策略契约不被循环直接碰，而是经 **Hook** 在生命周期点触发（这就是「Hook 注入」）。两条解耦规则因此一眼可见 ：① 所有外部箭头单向指向 core ；② 策略只能经 Hook 间接接入。（例外 ：`StopStrategy` 作为 `AgentLoop` 构造参数被每轮**直接调用**、不经 Hook，见 §5.4。）
...
- **Hook** 是注入机制 ——它定义「在什么时间点可以插入行为」（8 个生命周期点）。
- **策略** 是可替换的算法 ——同一个注入点上，换个实现就换了行为。最典型的是**上下文管理策略**，回答「上下文逼近窗口上限时、怎样让模型少看一些」：可以是「截断最旧的消息」、「清理已无用的工具结果」、「把更早的历史压缩成摘要」……这些都是可互换的算法（§2.5）。同一套机制还承载**权限策略**（这次工具调用放不放行 ：deny-first 管线 + 多档信任级别）与**停止策略**（这一轮要不要终止 ：按轮数 / token 预算 / 成本上限）。引入一项新最佳实践，往往就是加一个策略实现，核心循环一行不改。

一个上下文管理策略要生效，需要被一个 Hook 实现「包」起来 ：策略提供算法，Hook 负责在 `before_model` 这个时间点调用该算法。策略回答「做什么」，Hook 回答「何时做」。

### 4.3 举例验证「稳定核心」

这一节用**一个具体例子**来证明「稳定核心」——它**不是性能压测、也不需要真跑**，靠的是把例子写出来、逐行看它落在哪、core 要不要改。例子选 harness 里**最硬**的功能 ：上下文压缩（这是未来 v0.3 才做的，所以下面两段代码是 v0.3 的、不是 v0.1 要写的）。挑它来举例，是因为它动的是**消息历史本身**，最容易逼着你改核心数据结构与循环 ；最难的例子都成立，别的就都成立。

读下面的代码只需先记住 §5.1.1 的一个设计 ：core 把上下文拆成两样——`ctx.messages` 是**完整历史日志**（只追加、谁都不能删），`ctx.view()` 是**本轮真正发给模型的那一份**，核心循环里只有一句 `llm.chat(ctx.view())`。于是「压缩」的本质就只是 ：**往 view 里写一个精简版、而不碰日志**。下面两段都落在 `strategies/`——第一段是「算法」，第二段把算法**包成一个 Hook**（即 §4.2 的「Hook 包策略」）。

```python
# strategies/context/ —— 假想的 v0.3 压缩策略（仅用于举例，v0.1 不启用）
# 【算法】输入完整 messages + token 预算，输出「本轮该发给模型的精简列表」
class KeepRecentContext:
    """保留 pinned 与预算内的最近消息；更早的移出本轮视图。只读 messages、产出新列表。"""
    def reduce(self, messages: list[Message], budget_tokens: int) -> list[Message]:
        kept, used = [], 0
        for m in reversed(messages):                       # 从最新往回留
            if m.pinned or used < budget_tokens:           # pinned(system) 无条件留；其余留到预算用完
                kept.append(m); used += m.token_estimate or 0
        return list(reversed(kept))                        # 被移出的消息仍在 ctx.messages，只是不进本轮请求

# 【接线】把上面的算法「包」成一个 Hook —— 这就是 §4.2 「Hook 包策略」的实物
class ContextHook(BaseHook):
    ...
    def before_model(self, ctx: Context) -> None:          # 在「模型调用前」这个时间点触发
        ctx.set_view(self._s.reduce(ctx.messages, self._budget))   # 只写投影 view，绝不碰日志 messages
```

启用方式只是装配时多传一个 hook ：`Agent(..., hooks=[ContextHook(KeepRecentContext(), 8000)])`，其它一概不动。

**自检结论**：接入这个压缩策略，`core/loop.py`、8 个 Hook 签名、`Message/Context` 字段**一行都不改**——新代码全部落在 `strategies/` 那两个类里。这证明「稳定核心」在上下文工程这个最硬的场景下成立。它反过来也解释了**为什么 §5.1 的 `view()` 与 `pinned/token_estimate` 这些占位字段必须在 v0.1 就位** ：没有 `view()`，压缩就只能去删 `messages`（毁掉历史）或改核心循环——core 当场就保不住了。这些字段现在空着不用，却是「将来 core 不改」的物理前提。
...
class MaxTurnsStop:                                  # 不继承 StopStrategy：结构对上即满足契约（§1.5.1）
    def __init__(self, max_turns: int = 20): self.max_turns = max_turns
    def should_stop(self, ctx: Context, turn: int) -> StopReason | None:
        return StopReason.MAX_TURNS if turn >= self.max_turns else None

# 想同时启用多个停止条件？包一个组合策略 —— 它自己也满足 StopStrategy（同样不继承）
class CompositeStop:
    def __init__(self, *strategies): self._ss = strategies
    def should_stop(self, ctx, turn):
        for s in self._ss:
            r = s.should_stop(ctx, turn)
            if r is not None:                        # 谁先命中用谁的 StopReason
                return r
        return None
...
本章示例都是 **v0.1**（命令行 + `@tool` 工具 + in-memory），也是当前唯一能真正跑通的版本。v0.2+ 的 MCP / Skill / subagent **用法示例**待各版本落地后再补；它们的**设计意图**见 §9 路线图、§1.4 全模块清单、§12.2。想知道**怎么往框架里加东西**（自定义工具 / 策略 / 能力实现），见 §11.3「如何扩展框架」。
...
_（待 v0.2 落地后补充 ：MCP 工具适配器接入、Skill 渐进式加载的配置与用法。设计意图先见 §9 路线图 / §1.4 / §12.2。）_
...
一句话 ：**新增能力 = 写一个满足契约的实现 + 在装配层接上，核心循环与 8 个 Hook 签名都不动。** v0.2 / v0.3 的 Skill、subagent、权限管线等都按这个套路加，具体设计见 §9 路线图与各版本的实现级设计文档。
...
**风险 3 ：「易于理解」与「Hook 抽象」存在张力。** 缓解 ：v0.1 默认配置不带任何 hook；且 §4.3 用一个假想的 v0.3 压缩策略举例验证了这层抽象，证明 core 不改即可承载——抽象成本真实存在，但已验证可控。

### 14.2 范围与时间风险

**风险 4 ：v0.3 的 harness 工程量被低估。** v0.3 要在同一版本里落上下文管理、权限系统、熔断器、subagent **四个相互独立的子系统**，每个都有自己的设计空间——尤其上下文工程是开放问题（截断 / 清理 / 摘要多策略并存），最易膨胀。把四者压在一个版本里，极易因某一项卡壳而拖垮整版节奏。缓解 ：① 拆成 **v0.3.1（上下文 + 权限）** 与 **v0.3.2（熔断 + subagent）**，各自独立可交付、独立验收，先交付前者再启动后者 ；② §4.3 已用一个假想压缩策略**举例验证**过「core 一行不改」，所以 v0.3 的全部工程量都是 `strategies/` 内的**增量**、不回头动核心——风险被物理锁在策略层，不会外溢成跨层重构。
