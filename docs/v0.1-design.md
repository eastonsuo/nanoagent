...
>
> **总纲与契约（Contract）仍以 `DESIGN.md` 为准**（尤其第 5 章核心数据结构与接口契约）；本文不重复定义契约，只在其上展开实现，按下文的依赖顺序逐模块推进。各模块的实时落地进度见 README 的状态/路线图与 `_internal/PROGRESS.md`，不写进本设计文档。
...
- **第一部分 · 设计（§3）**：讲清 **core 的设计**——稳定核心为什么这样切（事件日志 + `view()` 投影、~30 行循环、8 个 Hook、能力 / 策略契约 + 单向依赖）。重「为什么」，完整字段/签名指向 [DESIGN §5](DESIGN.md#5-核心数据结构与接口契约)、不重述。
...
```mermaid
flowchart TD
    core["core/ · 依赖根<br/>数据结构 Message·Context·ToolCall…<br/>能力契约 LLMClient·Tool·MemoryBackend<br/>策略契约 Context·Permission·Stop<br/>机制 8 点 Hook · StopReason"]
    ...
    classDef root fill:#aaffaa
    class core root
```

**逐步走读**：绿色 `core/` 是所有节点指向的**依赖根**（地基），它本身只装**契约**——节点里列的三族（数据结构 / 能力契约 / 策略契约）正是每条入边标注的「实现 XX 契约」所指；完整签名以 [DESIGN §5](DESIGN.md#5-核心数据结构与接口契约) 与本文 §3 速查表为准，图里只列名。`tools / llm / memory / strat` 是**能力与策略实现**，各自实现 core 里的一个 Protocol，彼此不依赖；`loop.py` 把这些契约编排成 ReAct 循环；`api.py` 是唯一「知道全部模块」的装配层，把字符串模型名解析成 `LLMClient`、把工具/策略/hook 组装进 `AgentLoop`；`cli/` 再在 api 之上包一层 REPL。`单测`不是末尾一个阶段，而是**随每个模块同步写**——用 echo 客户端（不打真实网络）在本地跑通该模块路径，每加一块都保持「本地可运行 + 有覆盖」。
...
- **能力 / 策略契约 + 单向依赖**：`LLMClient` / `Tool` / `MemoryBackend` + 三个策略 Protocol 都属 core；**`core/` 不 import 任何外层**（import-linter 守，契约已进 `pyproject.toml`）。→ [DESIGN §5.2 能力契约](DESIGN.md#52-能力契约-protocol)、[§8.1 依赖防线](DESIGN.md#81-依赖防线ci-强制)。
...
**要做什么**：把前面所有契约编排成 ReAct 循环。这是全项目的心脏，也是「核心主体只含 ReAct 主干、不含任何具体策略代码」这条硬约束的落点（[DESIGN §5.5](DESIGN.md#55-agentloop-核心循环)）——「~30 行」是这条规则的标尺，不是要逐行卡的硬指标。
...
### 8.5 主体自检（行数是标尺、不是硬指标）

`AgentLoop.run` 主体（§8.1 那段）的硬要求是**只含 ReAct 主干、不含任何具体策略代码**——所有 harness 行为都在 `_emit / _gate / stop.should_stop` 背后；`_emit/_gate/_invoke/_denial_message` 是辅助函数，不铺进主体。「~30 行」是这条规则的**标尺**：验收时主体若明显超出，应排查是不是策略代码漏进了主循环，而不是机械卡行数。
...
签名与 [DESIGN §5.6](DESIGN.md#56-顶层装配契约agent-与-chatsession) 一致——`Agent` 与 `ChatSession` 是架在同一个无状态 `AgentLoop` 上的**两个平级入口**（非包含关系，区别只在各管的 `Context`，详见 §5.6）。三个要点：
...
- **`Agent`**（一次性任务、跨 run 不留对话状态；单次 run 内仍是多轮 ReAct）：`run(prompt)` 内部 `Context()` → 把 `system_prompt` 包成 `pinned` 的 system `Message`、prompt 包成 user `Message` → 调 `AgentLoop.run`。
- **`ChatSession`**（多轮对话、有记忆）：复用同一个无状态 `AgentLoop` + 持续累积的同一个 `Context`（状态归 Context、编排归 Loop，[DESIGN §14.3](DESIGN.md#143-v01-实现前的开放问题已定默认)-Q3）。
