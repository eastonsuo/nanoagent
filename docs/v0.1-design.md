...
DESIGN.md 回答「nanoagent 由哪些部分组成、为什么这样切分」；本文回答「v0.1 这一周，**每个模块具体怎么写**」。

组织方式是**按依赖顺序**（不是按代码分层、也不绑定「第几天」）：从被依赖的底层往上叠，读到 `loop.py` 那章时，它依赖的 `tools / llm / memory / strategies` 都已在前几章就位。**时长不强制**——按依赖推进即可，「一周」只是粗略参考，不是工期承诺。

每章固定回答四个问题：**这个模块要做什么 → 关键签名长什么样 → 有没有非平凡算法 → 怎么连同单测一起写**。单测与代码**同步写**、用 echo 客户端在本地跑（不依赖网络与 API key），目标是每完成一个模块都保持「本地可运行 + 有覆盖」，而不是把测试堆到最后。涉及真实设计取舍的图配三段式解读；只是展示静态结构的图只走读一遍。

---

## 2. v0.1 全景：造什么、按什么顺序造

v0.1 的成品是「终端里输入 `nanoagent` 就能对话、能调工具、能看 token 用量」的命令行 agent。它要做对的是**可演进性**，不是功能完整——判据是：v0.3 接真正的 harness 时 `core/` 一行不改（DESIGN §10.1）。
...
**当前方案为何如此**：顺序的唯一约束是「先做被依赖的」——`loop` 依赖四个能力/策略契约的**实现**才能跑通最小循环，所以它排在 core（只定**契约**、不含 loop 实现）与四个实现**之后**。`api` 依赖面最广所以最后装配，但它**不在 `core/` 内**，因此「core 不依赖外层」这条线没破。能力实现层（`tools/llm/memory`）与策略层（`strat`）分开，是因为前者被循环**直接调用**、后者经 Hook **间接注入**（DESIGN §6）。这是**依赖序、不是日历**——每块多久完成不重要，先后关系才是约束。
...
`core/` 已实现（`message / context / protocols / hooks / stop / errors`），完整定义见 DESIGN §5。这里只用两张表把后续各章会反复引用的两组契约钉死，便于对照。
...
`before_compact` 在 v0.1 是「声明但 `AgentLoop` 不 emit」的预留点——与 `Message.token_estimate` 等占位字段同属「v0.1 声明、暂不驱动」，避免日后新增注入点成为破坏性变更（DESIGN §4.1、§14.1）。
...
`FunctionTool` 是一个 `@dataclass`，`__call__` 直接转调原函数。命名规则强制 `^[a-z][a-z0-9_]*$` 且 ≤64 字符，默认取函数名（天然 snake_case），**重名在注册表 `register()` 时报错**；v0.1 不引入 namespace 前缀（留到 Skill / MCP 多来源出现重名时，DESIGN §14.3）。
...
**当前方案为何如此**：用标准库 `inspect` + `typing` 而不是 pydantic——v0.1 的硬约束是核心依赖 ≤5、不引 pydantic（DESIGN §3.2、§12.3）。代价是类型覆盖窄（不做嵌套 model 校验、不解析 `list[T]` 的 item 类型），但 v0.1 的内置工具参数都是 `str/int` 这类标量，够用；参数描述靠约定（在 docstring 里用简单格式写），不强行解析 Google/Numpy docstring 风格。
...
`registry.py` 维护一个 `name -> Tool` 字典，`register()` 时校验重名；loop 拿到的是 `list[Tool]`，内部转 `{t.name: t}`（与 DESIGN §5.5 `AgentLoop.__init__` 一致）。v0.1 内置工具：
...
两个易踩的坑，必须做对：① **`tool_calls` 的唯一权威源是 assistant 消息自带的 `tool_calls`**（DESIGN §5.1 已删掉 `LLMResponse` 上冗余的 `tool_calls` 字段，避免和 message 内的重复、导致 OpenAI 400）；② **`tool` 消息必须带 `tool_call_id` 且与前一条 assistant 的某个 `tool_calls[].id` 配对**，否则下一轮请求 400——这正是 loop 里 `_denial_message` 也要产出带 `call_id` 的 tool 消息的原因（§8.4）。

`resolve_model` 读 API key、选 endpoint 的逻辑**不在这里、在 api 层**（DESIGN §5.6）——本客户端只接收已解析好的 `model` 名与（可选）`base_url`，保持 core 与客户端对「字符串模型名」无感。
...
`retrieve` 在 v0.1 不做向量/语义检索（那是 semantic memory，v0.2+ 基于向量），只朴素返回最近若干条——契约对、实现简陋，符合 v0.1「形状定对、实现可最简」的原则（DESIGN §10.1）。
...
**要做什么**：落 DESIGN §5.4 的三个策略 Protocol 各一个默认实现。它们都放 `strategies/`（实现属策略层），core 只见协议。三个都极简——v0.1 默认配置「不带 hook、纯 ReAct」就靠它们退化：
...
# strategies/stop/max_turns.py —— 只看轮数（DESIGN §5.4）
class MaxTurnsStop:
...
注意三者的接线方式不同（DESIGN §5.4）：`ContextStrategy` / `PermissionStrategy` 要被一个 **Hook** 包起来才生效（分别在 `before_model` / `before_tool` 时间点调用）；而 `MaxTurnsStop` 是 `AgentLoop` 的**构造参数**、每轮轮首直接被调，不算 hook——因为停止判定是控制流主干。v0.1 默认就用 `MaxTurnsStop(20)`，`NoopContext` / `AllowAll` 只有在显式启用对应 hook 时才介入（默认不启用，保持纯 ReAct）。
...
**要做什么**：把前面所有契约编排成 ReAct 循环。这是全项目的心脏，也是「核心约 30 行、不含任何具体策略代码」这条硬约束的落点（DESIGN §5.5）。

### 8.1 主体（与 DESIGN §5.5 一致，8 点 Hook 版）
...
**还能怎么优化**：① 图中一轮内多个 `tool_calls` 是**串行**执行；当模型一次吐多个相互独立的调用时，串行拖慢延迟——可在 v0.4 用异步 `arun` 让独立调用并发（DESIGN §7、§14.4），当前同步优先是为可读性。② `before_compact` 现在是空预留点；v0.3 接压缩时，它的 emit 位置（loop 主动 emit vs 压缩策略内部触发）涉及「核心循环不该知道 compaction」的边界，需在写 v0.3 时单独定，不在 v0.1 拍。
...
**要做什么**：`api.py` 是唯一「知道全部模块」的装配层（不在 core 内，可 import 全部）；`cli/` 在其上做 REPL。这一层让 DESIGN §11 的 `Agent("gpt-4o-mini", tools=[...]).run("...").output` 能跑通。

### 9.1 api.py：resolve_model / Agent / ChatSession

签名与 DESIGN §5.6 一致。三个要点：
...
- **`ChatSession`**（多轮）：复用同一个无状态 `AgentLoop` + 持续累积的同一个 `Context`（状态归 Context、编排归 Loop，DESIGN §14.3-Q3）。
...
命令行入口 `cli/main:main`（`pyproject` 的 `[project.scripts]` 把 `nanoagent` 映射到它）。REPL 用 `prompt-toolkit`，每轮把 `AgentResult` 渲染出来，并显示「N 轮 · X tokens」（来源：`AgentResult.turns` 与 `ctx.usage` 累计，DESIGN §5.5）。
...
**逐步走读**：环境变量优先 → 其次读 `~/.nanoagent/config.toml` → 都没有则一次性引导 wizard 粘贴 key，并以 600 权限落盘。读 toml 用标准库 `tomllib`（Py3.11+）；写则手工拼字符串，**不引第三方 toml 写库**——守住核心依赖 ≤5 这条约束（DESIGN §14.3）。
...
验收标准（DESIGN §9）：**核心 < 500 行；测试覆盖 > 70%；命令行可对话 / 调工具 / 显示 token 用量；支持 OpenAI 及兼容接口。**
...
两个示例（DESIGN §10.3）：`hello agent`（最小对话）与 `research agent`（带 web_search 工具的多轮）。前者证明循环跑通，后者证明工具链路与多轮上下文累积。

---

## 11. 已知债务

诚实列出 v0.1 知道但当前不修的点（部分见 DESIGN §14.4）：
...
- **面向第三方前端的事件流 / 中断接口尚未设计**（DESIGN §14.4）。
